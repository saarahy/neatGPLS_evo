...
def energy_coolng(n_corr,p, problem, name_database):
...
def main(n_corr, p,problem):
    name_database="ConcreteData.txt"
    direccion="./data_corridas/%s/train_%d_%d.txt"
    energy_coolng(n_corr,p, problem,name_database)
    ...
    pop, log = neatGPLS.neat_GP_LS(pop, toolbox, cxpb, mutpb, ngen, neat_alg, neat_cx, neat_h,
                                   neat_pelit, funcEval.LS_flag,
                                   LS_select, cont_evalf, num_salto, SaveMatrix,GenMatrix,
                                   pset,n_corr, p, params,
                                   direccion,problem,testing,version=version,stats=mstats,
...
if __name__ == "__main__":
    problem="Concrete2"
    number=111109
    d = './Timing/%s/timing_Concrete_%s.txt' % (problem, number)
    ...
    while n < 31:
        begin_p = time.time()
        main(n, number, problem) #1701 cx-sub 17012
        n += 1
        end_p=time.time()
